...
var Engine = (function(global) {
    ...
    //player和enemies一共有左/右/上/下四种情况进行碰撞,碰撞后重启游戏;
    function checkCollisions(){
        allEnemies.forEach(function(enemy){
          var a = enemy.x - player.x,
              ...
              c = enemy.y - player.y,
              d = player.y - enemy.y;
          if ( a < 55 && b< 65 && c < 50 && d < 70) {
              reset();
          }
        });
    };
    //player到达小河后,游戏过关，画面出现过关提示，且按下spacebar可以重新开始;
    function achieveGoal(){
        if (player.y < 0){
          textDrawer('CONGRATULATIONS, YOU WON!',15,260,'rgb(255,95,5)');
          textDrawer('PRESS "SPACE" TO RESTART',50,430,'rgb(250,250,250)');
          if (player.status == 'reset_game' ) {
            reset();
          }
        }
    };
    //textDrawer是在canvas中写入不同颜色文字的函数
    function textDrawer(text,x,y,color){
        ctx.save();
   ...
   //当player不在小河范围以内，更新player位置；当player到达小河后（游戏通关），player位置不更新（停止运动）
        if (player.y > 0){
            player.update(dt);
        }
    ...
    function render() {
        ...
        renderEntities();
        //在canvas上方添加标题。
        textDrawer('ARCADE GAME',140,0,'rgb(250,100,5)');
    }
     ...
     */
    function reset() {
        player.x = 101*Math.floor(Math.random()*5);
        player.y = 318+83*Math.floor(Math.random()*2);
    }
